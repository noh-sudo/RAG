...
```
**검증된 결과** (원본 1,118건 전체, 실제 Ollama `bge-m3` 서버로 실행 — 가짜 임베딩 아님): 1,118/1,118건 인덱싱 완료(소요 약 15분, CPU 임베딩 기준), 컬렉션 `minutes_chunks`에서 임베딩 차원 1024 확인. `meta.json`에 `scope.generation_no=[15, 16, 17, 18, 19, 20, 21]`, `period_from="1996-07-15"`, `period_to="2022-09-21"` 정상 산출 확인.
